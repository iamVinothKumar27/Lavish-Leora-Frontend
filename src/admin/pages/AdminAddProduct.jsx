...
const INITIAL = {
  name: '', description: '', price: '', category: 'Women', subcategory: '',
  stock: '', sizes: [], featured: false, newArrival: false, koreanStyle: false,
};
...
export default function AdminAddProduct() {
  ...
  const [urlInputs, setUrlInputs] = useState(['']); // typed URL strings
  const [imageFiles, setImageFiles] = useState([]);  // File objects
  const [filePreviews, setFilePreviews] = useState([]); // object URLs for preview
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-card p-6 space-y-5">
          ...
          <div>
            ...
            <input name="name" value={form.name} onChange={handleChange} required placeholder="e.g. Korean Floral Maxi Dress" className="input-field" />
          </div>

          <div>
            <label className="text-sm font-medium text-gray-700 mb-1.5 block">Description *</label>
            <textarea name="description" value={form.description} onChange={handleChange} required rows={4} placeholder="Detailed product description..." className="input-field resize-none" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1.5 block">Price (₹) *</label>
              <input name="price" type="number" value={form.price} onChange={handleChange} required min="0" placeholder="e.g. 1299" className="input-field" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1.5 block">Stock Quantity *</label>
              <input name="stock" type="number" value={form.stock} onChange={handleChange} required min="0" placeholder="e.g. 50" className="input-field" />
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-card p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {sizes.map((sz) => (
              <button
                key={sz} type="button" onClick={() => toggleSize(sz)}
        ...
        <div className="bg-white rounded-2xl shadow-card p-6">
          ...
          <div className="mb-5">
            ...
            <p className="text-xs text-gray-400 mb-3">Paste URLs from Unsplash, Pexels, or any public image host.</p>
            <div className="space-y-3">
              {urlInputs.map((url, i) => (
                <div key={i} className="flex gap-2 items-start">
                  ...
                  {urlInputs.length > 1 && (
                    <button type="button" onClick={() => removeUrlField(i)} className="mt-2 text-red-400 hover:text-red-600 text-lg leading-none" title="Remove">×</button>
                  )}
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mb-3">JPG, PNG, WEBP or GIF — max 5 MB each.</p>
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
        ...
        <div className="bg-white rounded-2xl shadow-card p-6">
          ...
          <div className="space-y-3">
            {[
              { name: 'newArrival', label: 'New Arrival', desc: 'Show in New Arrivals section on homepage' },
              { name: 'featured', label: 'Featured', desc: 'Highlight as a featured product' },
              { name: 'koreanStyle', label: 'Korean Style', desc: 'Show in Korean Style collection' },
            ].map(({ name, label, desc }) => (
              <label key={name} className="flex items-start gap-3 cursor-pointer group">
                <input type="checkbox" name={name} checked={form[name]} onChange={handleChange} className="mt-0.5 w-4 h-4 accent-primary-600" />
                <div>
        ...
        <div className="flex gap-3 pb-8">
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-full font-semibold text-sm transition-all ${
              loading ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-primary-600 hover:bg-primary-700 text-white hover:shadow-hover'
            }`}
          >
